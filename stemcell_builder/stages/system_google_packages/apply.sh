...
then
  ...
  run_in_chroot $chroot "dpkg -i /tmp/google-compute-daemon_1.3.3-1_all.deb || true"
  pkg_mgr install
...
then
  ...
  run_in_chroot $chroot "yum -y install /tmp/google-compute-daemon-1.3.3-1.noarch.rpm"
